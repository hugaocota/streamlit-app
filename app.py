...
if menu_option == "Script de Venda":
    ...
    if textos_dict:
        ...
        texto_saudacao = texto_saudacao.replace(
            "{cliente_nome}", cliente_nome).replace("{vendedor_nome}", vendedor_nome)

        texto_introducao = textos_dict.get(
            'introducao', 'Texto padrão de introdução')
        texto_ramo = textos_dict.get(
            'pergunta_ramo', 'Texto padrão de pergunta sobre ramo')
        texto_maquina = textos_dict.get(
            'pergunta_maquina', 'Texto padrão de pergunta sobre máquina')
        ...
        maquina_cliente = st.selectbox(
            "Selecione a Máquina:", abas, index=abas.index(default_machine))
